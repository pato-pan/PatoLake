...
# however, don't put the all the blame on exiftool. Websites like to assign the wrong extensions to their filenames for unknown reasons I will likely not agree with -_-. So use the below :) and create a text file called fixext.txt with all the directories. One group at at time
